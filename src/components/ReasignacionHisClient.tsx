...
export function ReasignacionHisClient(props: {
  ...
  const [diresa, setDiresa] = useState<string>("LORETO");
  ...
  async function openModal() {
    ...
    if (!diresa.trim()) {
      setError("Ingresa la DIRESA esperada (por defecto: LORETO).");
      return;
    }
    if (!vols.length) {
  ...
  function confirm() {
    ...
    const diresaInput = f.querySelector<HTMLInputElement>('input[name="diresa"]');
    ...
    if (diresaInput) diresaInput.value = diresa.trim();
    setModal(false);
    f.requestSubmit();
  }
    ...
    <form ref={formRef} action={props.action} className="mt-5 flex flex-col gap-4">
      ...
      {props.isSuperAdmin ? (
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4 md:items-end">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-900">DIRESA esperada</label>
            <input
              value={diresa}
              onChange={(e) => setDiresa(e.target.value)}
      ...
      <input type="hidden" name="diresa" defaultValue={diresa.trim()} />
      ...
      <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4">
        ...
        <div className="mt-1 text-sm text-zinc-600">
          Revisa la tabla <span className="font-semibold">atenciones</span> (último <span className="font-semibold">periodo</span>{" "}
          por DNI). Si el último registro tiene <span className="font-semibold">diresa</span> distinta a la indicada, se reasigna
          el niño a voluntarios (seleccionados) de manera equitativa. Si diresa coincide o no hay atenciones, no se hace ningún cambio.
        </div>
      ...
      {modal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white ring-1 ring-black/10">
            <div className="p-5">
              <div className="text-base font-semibold text-zinc-900">Confirmar reasignación por HIS</div>
              <div className="mt-1 text-sm text-zinc-600">
                Etapa: <span className="font-semibold">{etapa}</span> · DIRESA esperada:{" "}
                <span className="font-semibold">{diresa.trim()}</span> · Voluntarios seleccionados:{" "}
              ...
              <label className="mt-4 flex items-start gap-3 text-sm text-zinc-800">
                ...
                <span>
                  Sobrescribir asignaciones existentes en la etapa seleccionada (solo para los casos con DIRESA distinta).
                  <div className="mt-1 text-xs text-zinc-500">
                    Si no está marcado, solo reasigna niños sin asignación en el mes seleccionado.
                  </div>
                </span>
              </label>
            </div>
