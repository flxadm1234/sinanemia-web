...
export function VisitasExcelImportClient(props: { canEditConfig: boolean }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch(`/api/visitas/jobs/${encodeURIComponent(jobId)}`, {
          cache: "no-store",
        });
  ...
  return (
    <div className="rounded-2xl bg-white ring-1 ring-black/5 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-sm font-semibold text-zinc-900">Carga de Excel (Visitas domiciliarias)</div>
      ...
      <div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4">
        ...
        {job?.message ? <div className="mt-3 text-xs text-zinc-600">{job.message}</div> : null}
      </div>
    </div>
  );
}
