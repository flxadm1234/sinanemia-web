...
export function PlantillaInformesClient(props: {
  ...
  const [pieCargo, setPieCargo] = useState("");
  ...
  const load = async (params: { q: string; page: number; ubigeo: string }) => {
    ...
    try {
      ...
      const res = await fetch(`/api/informe-configuracion?${sp.toString()}`, { cache: "no-store" });
  ...
  const startEdit = (r: InformeRow) => {
    setMode("edit");
    setEditingId(r.id);
    setFormUbigeo(String(r.ubigeo ?? ""));
    setEntidadNombre(String(r.entidad_nombre ?? ""));
    setGerenciaNombre(String(r.gerencia_nombre ?? ""));
    setLemaAnual(String(r.lema_anual ?? ""));
    setPieCargo(String(r.pie_cargo ?? ""));
    setCiudad(String(r.ciudad ?? ""));
  ...
  const onSave = async () => {
    ...
    try {
      ...
      fd.append("entidad_nombre", entidadNombre);
      fd.append("gerencia_nombre", gerenciaNombre);
      fd.append("lema_anual", lemaAnual);
      fd.append("pie_cargo", pieCargo);
      fd.append("ciudad", ciudad);
      fd.append("activo", activo ? "1" : "0");
      if (logo) fd.append("logo", logo);

      const xhr = new XMLHttpRequest();
      const url = mode === "edit" && editingId ? `/api/informe-configuracion/${editingId}` : "/api/informe-configuracion";
      ...
      const p = await new Promise<any>((resolve, reject) => {
        ...
        xhr.upload.onprogress = (evt) => {
          ...
          setSavePct(Math.max(0, Math.min(100, pct)));
        };
        xhr.onload = () => {
          const ok = xhr.status >= 200 && xhr.status < 300;
          if (!ok) {
            const err = (xhr.response as any)?.error || xhr.statusText || "Error guardando";
            reject(new Error(String(err)));
            return;
          }
          resolve(xhr.response);
        };
        xhr.onerror = () => reject(new Error("Error de red guardando"));
        xhr.send(fd);
      });

      if (!p?.row) throw new Error(String(p?.error || "No se pudo guardar"));
      setSavePct(100);
      resetForm();
      ...
      setSaving(false);
      setTimeout(() => setSavePct(0), 600);
    }
  };

  const onDelete = async (id: number) => {
    if (!confirm("¿Eliminar esta configuración? Esta acción no se puede deshacer.")) return;
    setDeletingId(id);
    setListError("");
    try {
      const res = await fetch(`/api/informe-configuracion/${id}`, { method: "DELETE" });
      const data = await res.json();
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-2xl bg-white ring-1 ring-black/5 p-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold text-zinc-900">Encabezado de informe</div>
            <div className="mt-1 text-sm text-zinc-600">
              {mode === "edit" ? "Editando configuración" : "Registrar nueva configuración"}
            </div>
          </div>
          {mode === "edit" ? (
            <button
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="rounded-2xl bg-white ring-1 ring-black/5 p-4">
            ...
            <datalist id="ubigeo-options">
              {ubigeoOptions.map((u) => (
                <option key={u} value={u} />
              ))}
            </datalist>
            <div className="mt-2 text-xs text-zinc-500">Define a qué distrito aplica el encabezado.</div>
          </div>

          <div className="lg:col-span-2 rounded-2xl bg-white ring-1 ring-black/5 p-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <label className="block text-sm font-medium text-zinc-900">Entidad</label>
                <input
                  type="text"
                  value={entidadNombre}
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-900">Gerencia</label>
                <input
                  type="text"
                  value={gerenciaNombre}
                  onChange={(e) => setGerenciaNombre(e.target.value)}
                  className="mt-2 w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                  placeholder="Ej: Gerencia de Desarrollo Social"
                  disabled={saving}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-zinc-900">Lema anual</label>
                <input
                  type="text"
                  value={lemaAnual}
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-900">Ciudad</label>
                <input
                  type="text"
                  value={ciudad}
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-zinc-900">Pie / Cargo</label>
                <input
                  type="text"
                  value={pieCargo}
                  onChange={(e) => setPieCargo(e.target.value)}
                  className="mt-2 w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                  placeholder="Ej: Actor Social del Compromiso 1"
                  disabled={saving}
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-zinc-900">Logo (PNG/JPG/WEBP/GIF)</label>
                <input
                  type="file"
                  ...
                  disabled={saving}
                />
                {logo ? <div className="mt-1 text-xs text-zinc-500">Seleccionado: {logo.name}</div> : null}
              </div>
            ...
            {saving || savePct ? (
              <div className="mt-4">
                <div className="h-2 w-full overflow-hidden rounded-full bg-zinc-200">
                  <div className="h-full bg-blue-700" style={{ width: `${savePct}%` }} />
                </div>
                <div className="mt-2 text-xs text-zinc-600">{saving ? `Guardando... ${savePct}%` : ""}</div>
              </div>
            ) : null}
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-white ring-1 ring-black/5 p-5">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-sm font-semibold text-zinc-900">Configuraciones registradas</div>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Buscar por entidad/gerencia/ciudad/ubigeo..."
                className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm"
              />
            </div>
          </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-zinc-50 text-left text-xs font-semibold text-zinc-700">
              <tr>
                ...
                <th className="px-4 py-3">Lema</th>
              ...
              {loadingList ? (
                ...
                rows.map((r) => (
                  <tr key={r.id} className="text-zinc-900">
                    ...
                    <td className="px-4 py-3">{r.entidad_nombre || "—"}</td>
                    <td className="px-4 py-3">{r.gerencia_nombre || "—"}</td>
                    <td className="px-4 py-3">{r.lema_anual || "—"}</td>
                    <td className="px-4 py-3">{r.ciudad || "—"}</td>
                    <td className="px-4 py-3">
                      {r.logo_path ? (
                        <img
                          src={`/api/informe-configuracion/${r.id}/logo`}
                          alt="logo"
                          className="h-8 w-16 rounded-md border border-zinc-200 object-contain bg-white"
                        />
                      ) : (
                        "—"
                      )}
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <div className="text-xs text-zinc-500">Página {page} de {totalPages}</div>
          <div className="flex items-center gap-2">
            <button
              type="button"
