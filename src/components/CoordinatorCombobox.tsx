...
export function CoordinatorCombobox(props: {
  name: string;
  defaultValue?: string;
  ubigeo?: number | null;
  disabled?: boolean;
  ...
  const [options, setOptions] = useState<CoordOption[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/coordinadores?${params.toString()}`, {
      signal: controller.signal,
      cache: "no-store",
    })
      .then(async (r) => {
        if (!r.ok) return [];
        return (await r.json()) as CoordOption[];
      })
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      >
        {selected ? `${selected.nombre} (${selected.dni})` : "Seleccionar coordinador"}
      </button>
